...
describe('POST /todos', () => {
  ...
  it('should not create todo with invalid body data', (done) => {
    //send empty object, expect 400, todos length should be 0
    request(app)
      .post('/todos')
      .send({})
      .expect(400)
...
describe('PATCH /todos/:id', () => {
  ...
  it('should clear completeAt when todo is not completed', (done) => {
    var hexId = todos[1]._id.toHexString();
    var text = "test update feature 222";
    var completed = false;
    request(app)
      .patch(`/todos/${hexId}`)
      .send({text, completed})
...
describe('POST /users', () => {
  ...
  it('should not create user if email in use', (done) => {
    // send an already email in use, expect 400
